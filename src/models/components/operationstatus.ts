/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { ClosedEnum } from "../../types/enums.js";
import * as z from "zod";

export const OperationStatus = {
    Running: "RUNNING",
    Completed: "COMPLETED",
    CompletedWithWarning: "COMPLETED_WITH_WARNING",
    Failed: "FAILED",
} as const;
export type OperationStatus = ClosedEnum<typeof OperationStatus>;

/** @internal */
export const OperationStatus$inboundSchema: z.ZodNativeEnum<typeof OperationStatus> =
    z.nativeEnum(OperationStatus);

/** @internal */
export const OperationStatus$outboundSchema: z.ZodNativeEnum<typeof OperationStatus> =
    OperationStatus$inboundSchema;

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace OperationStatus$ {
    /** @deprecated use `OperationStatus$inboundSchema` instead. */
    export const inboundSchema = OperationStatus$inboundSchema;
    /** @deprecated use `OperationStatus$outboundSchema` instead. */
    export const outboundSchema = OperationStatus$outboundSchema;
}
