 ...
 */
export async function primaryNetworkTransactionsListLatestPrimaryNetworkTransactions(
    client$: GlacierSDKCore,
    request: operations.ListLatestPrimaryNetworkTransactionsRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.ListLatestPrimaryNetworkTransactionsResponseBody,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, operations.ListLatestPrimaryNetworkTransactionsResponseBody$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
