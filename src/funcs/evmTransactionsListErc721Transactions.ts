 ...
 */
export async function evmTransactionsListErc721Transactions(
    client$: GlacierSDKCore,
    request: operations.ListErc721TransactionsRequest,
    options?: RequestOptions
): Promise<
    Result<
        components.ListErc721TransactionsResponse,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const [result$] = await m$.match<
        ...
        | ConnectionError
    >(
        m$.json(200, components.ListErc721TransactionsResponse$inboundSchema),
        m$.fail(["4XX", "5XX"])
    )(response);
    if (!result$.ok) {
        return result$;
