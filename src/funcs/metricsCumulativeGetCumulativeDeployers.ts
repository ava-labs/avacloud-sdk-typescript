...
import { GetCumulativeDeployersServerList } from "../models/operations/getcumulativedeployers.js";
 ...
 */
export async function metricsCumulativeGetCumulativeDeployers(
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.GetCumulativeDeployersRequest$outboundSchema.parse(value$),
    ...
    const baseURL$ =
        options?.serverURL ||
        pathToFunc(GetCumulativeDeployersServerList[0], { charEncoding: "percent" })();

    const pathParams$ = {
        chain: encodeSimple$("chain", payload$.chain, { explode: false, charEncoding: "percent" }),
    };

    const path$ = pathToFunc("/v1/cumulative/deployers/{chain}")(pathParams$);
