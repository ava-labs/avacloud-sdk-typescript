...
import { GetCumulativeAddressesServerList } from "../models/operations/getcumulativeaddresses.js";
 ...
 */
export async function metricsCumulativeGetCumulativeAddresses(
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.GetCumulativeAddressesRequest$outboundSchema.parse(value$),
    ...
    const baseURL$ =
        options?.serverURL ||
        pathToFunc(GetCumulativeAddressesServerList[0], { charEncoding: "percent" })();

    const pathParams$ = {
        chain: encodeSimple$("chain", payload$.chain, { explode: false, charEncoding: "percent" }),
    };

    const path$ = pathToFunc("/v1/cumulative/addresses/{chain}")(pathParams$);
